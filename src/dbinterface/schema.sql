...
-- Per-winding electrical/control state for both TwoWindingTransformer and
-- ThreeWindingTransformer, mirroring POM's TransformerCircuit struct.
-- A TwoWindingTransformer references exactly one row here, and a
-- ThreeWindingTransformer references three (primary/secondary/tertiary).
-- Type-specific fields (control_limits, controlled_quantity_limits) fall
-- through to the generic `attributes` table as JSON.
CREATE TABLE transformer_circuits (
    ...
    regulated_bus_number INTEGER,
    number_of_tap_positions INTEGER,
    rating REAL,
    rating_b REAL,
    rating_c REAL,
...
-- TwoWindingTransformer holder rows. All electrical state lives on the
-- referenced `transformer_circuits` row -- the holder just carries name,
-- shunt, and admittance metadata (matches POM's TwoWindingTransformer).
CREATE TABLE two_winding_transformers (
    id INTEGER PRIMARY KEY REFERENCES entities (id) ON DELETE CASCADE,
    name TEXT NOT NULL UNIQUE,
    circuit_id INTEGER NOT NULL REFERENCES transformer_circuits (id) ON DELETE CASCADE,
    admittance_units TEXT,
    shunt_location TEXT
) strict;

-- ThreeWindingTransformer holder rows. Per-winding electricals live on the
-- three referenced `transformer_circuits` rows -- the pairwise mutual
-- impedances / base powers stay here to match POM's ThreeWindingTransformer.
CREATE TABLE three_winding_transformers (
    id INTEGER PRIMARY KEY REFERENCES entities (id) ON DELETE CASCADE,
    name TEXT NOT NULL,
    primary_circuit_id INTEGER NOT NULL REFERENCES transformer_circuits (id) ON DELETE CASCADE,
    secondary_circuit_id INTEGER NOT NULL REFERENCES transformer_circuits (id) ON DELETE CASCADE,
    tertiary_circuit_id INTEGER NOT NULL REFERENCES transformer_circuits (id) ON DELETE CASCADE,
